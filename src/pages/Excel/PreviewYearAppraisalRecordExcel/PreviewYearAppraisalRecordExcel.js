...
const PreviewYearAppraisalRecordExcel = () => {
  ...
  const onDocumentError = function (event) {
    ...
    console.error("[OnlyOffice 預覽] 年度考核文件錯誤", error);
   ...
   * ✅ 是否為歷史文件預覽
   *
   * 前端只帶 historyVersionId。
   * 真實歷史檔路徑由後端 getExcelFileVersionById 查 DB。
   */
   ...
   *
   * 範例：
   * office/excel/history/EmployeeYearAppraisalExcelHistory/xxx.xlsx
   */
  ...
  const buildCallbackUrl = ({
    ...
    return (
      `${onlyOfficeCallbackBaseUrl}/${AppraisalYearRecordExcelCallback}` +
  ...
  const buildDocumentEditor = ({
    ...
    const documentKey = isHistoryPreview
      ? `year_history_${documentId}_${historyVersionId}_${Date.now()}`
      : officeMode === "view"
  ...
  const openHistoryExcel = async ({
    ...
    const excelName = version?.version_file_name || "年度考核歷史文件.xlsx";
    ...
    console.log("OnlyOffice year history preview");
  ...
  const openMainExcel = async ({ documentId, loginUser }) => {
    ...
    if (success !== 1) {
      ...
      setLoading(false);
      setLoadError(result?.data?.message || "查無年度考核表");
      return;
    }
    ...
    setFallbackDownload({
      fileUrl: excelData?.document_file_url
        ...
        : encodeURI(`${mybaseUrl}/office/excel/EmployeeAppraisalExcelYear/${excelName}`),
      fileName: excelName,
    });
    ...
    console.log("OnlyOffice year mode =", officeMode);
  ...
  const getExcelNameFetch = async (documentId) => {
    try {
      ...
      console.error(e);
      setLoading(false);
      setLoadError(e?.response?.data?.message || e?.message || "讀取年度考核表失敗");
    }
  };
  ...
  useEffect(() => {
    ...
    if (!metadataStartedRef.current) {
      ...
      console.time(`[OnlyOffice 預覽] 年度考核文件資訊 ${documentId}`);
      getExcelNameFetch(documentId).finally(() => {
        console.timeEnd(`[OnlyOffice 預覽] 年度考核文件資訊 ${documentId}`);
      });
    }
    return undefined;
  ...
  useEffect(() => {
    if (leaseReady && preparedDocumentEditor) {
      console.info("[OnlyOffice 預覽] 年度考核租約與文件資訊完成，開始載入編輯器");
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      const retryKey = `onlyoffice-initial-retry:${window.location.href}`;
      if (!window.sessionStorage.getItem(retryKey)) {
        console.warn("[OnlyOffice 預覽] 年度考核載入逾時，自動重試一次");
        window.sessionStorage.setItem(retryKey, "1");
        window.location.reload();
        return;
      }
...
export default PreviewYearAppraisalRecordExcel;
