...
import CheckApprasialModal from '@/pages/Modal/checkApprasialModal';
import DeleteModal from '@/pages/Modal/deleteModal';
import MyMergeDocumentModal from '@/pages/Modal/MyMergeDocumentModal';
...
const EmployeeAppraisalUsually = () => {
  ...
  const currentRows = AppraisalTableSnap.AppraisalTable?.data || [];
  ...
  /**
   * ✅ rowSelection / 批次按鈕顯示權限
   *
   * 平時考核也比照 EmployeeBonusSurvey：
   * - 人事 / 管理者
   * - 總幹事
   * - 秘書
   */
  ...
  const getFlowTypeKeyByRow = (row) => {
    ...
    return 'usually_appraisal';
  };

  /**
   * ✅ 重新查詢平時考核列表
   */
  const safeReloadAppraisalTable = async (year) => {
    setTableLoading(true);
    setTableRowKey([]);
    setcheckData([]);

    try {
      await Promise.resolve(TableModel.setAppraisalTable(year));
    } finally {
      setTimeout(() => {
  ...
  const reloadAppraisalTable = () => {
    const year = DropDownModel.getSelectedYear('normal');

    safeReloadAppraisalTable(year);
  };
  ...
  const checkExcel_Blank = async (excelName) => {
    const result = await checkAppraisalRecordError(excelName);
          ...
          return (
            ...
            >
              ...
              <Button
                ...
                onClick={() => {
                  window.open(
                    `${ROUTENAME.PreviewAppraisalRecordExcel}?DocumentId=${item?.excel_id}`,
                    '_blank',
                  );
                }}
              >
    ...
    <div className="EditTableStyle">
      ...
      >
        ...
        <div style={{ minWidth: '180px', textAlign: 'right' }}>
          {(initialState?.user?.admin_type === '0' ||
            ...
            <Button
              type="primary"
              disabled={isExcelTypeDisabled(1)}
              style={{
                backgroundColor: isExcelTypeDisabled(1)
                  ? '#d9d9d9'
                  : '#389e0d',
                color: isExcelTypeDisabled(1) ? '#666' : '#fff',
                borderColor: isExcelTypeDisabled(1)
                  ? '#d9d9d9'
                  : '#389e0d',
                cursor: isExcelTypeDisabled(1)
                  ? 'not-allowed'
                  : 'pointer',
              }}
              onClick={async () => {
                if (isExcelTypeDisabled(1)) return;
                if (!(await ensureExcelTypeIsOpen(1))) return;
                ...
                const handleMessage = (event) => {
                  ...
                  // ✅ 只處理平時考核建立成功通知
                  if (data?.type !== 'NORMAL_APPRAISAL_EXCEL_CREATED') return;

                  const currentYear = DropDownModel.getSelectedYear('normal');

                  safeReloadAppraisalTable(currentYear);

                  console.log('收到平時考核 Excel 建立完成通知，重新查詢列表：', {
                    currentYear,
                ...
                const newWindow = window.open(
                  ROUTENAME.CreateAppraisalRecordExcel,
                  '_blank',
                );
          ...
          headerTitle={
            ...
            >
              ...
              <Select
                ...
                value={dropDownSnap.getSelectedYear('normal')}
                onChange={(value) => {
                  DropDownModel.setSelectedYear('normal', value);

                  safeReloadAppraisalTable(value);
                }}
              />
      ...
      <ApprovalSignModal
        ...
        onPreviewHistory={(version) => {
          ...
          window.open(
            `${ROUTENAME.PreviewAppraisalRecordExcel}?${query.toString()}`,
            '_blank',
          );
        }}
      />
...
export default EmployeeAppraisalUsually;
