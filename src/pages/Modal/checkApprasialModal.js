...

      {
        blankArray.map((item) => {
          ...
          for (const scoreKey in scores) {
            ...
            // 不是主管卻填入
            if (scoreKey === "d8" && !isManager && scores[scoreKey]?.value!==null) {
              childDomArray.push(
                <>
            ...
            }else if(scoreKey === "d8" && !isManager && scores[scoreKey]?.value===null){
              childDomArray.push(
                <>
            ...
            //   是主管卻沒有填入
              ...
              childDomArray.push(
                ...
                <td>{scores[scoreKey]?.name}</td>
                  <td>{scores[scoreKey]?.value}
                    {scores[scoreKey]?.value===null &&<span style={{position:'absolute',color:'red',right:15,top:14 ,fontSize:'16px'}}>(未填)</span>}<
                  /td>
                </tr>
              )
            }
